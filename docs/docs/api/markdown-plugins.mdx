...
Leaf exports TypeScript types for better plugin development:
...
Type for remark plugins that process markdown AST.
...
// Simple plugin
const myPlugin: RemarkPlugin = () => (tree) => {
  // Transform tree
};

// Plugin with options
const myPluginWithOptions: RemarkPlugin = [myPlugin, { enabled: true }];
```

### RehypePlugin

Type for rehype plugins that process HTML AST.
...
// Simple plugin
const myRehypePlugin: RehypePlugin = () => (tree) => {
  // Transform tree
};
```
...
Type for table of contents items extracted during markdown processing.

```typescript
interface TocItem {
  text: string;   // Heading text
  id: string;     // Generated ID (slug)
...
Configuration interface for custom plugin systems.

```typescript
interface LeafPluginConfig {
