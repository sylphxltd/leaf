...
- ⚡ **Bun Runtime** - Faster builds with modern JavaScript runtime
- 🎨 **Modern Tooling** - TypeScript-first development
- 📦 **Lightweight** - Minimal bundle with Sylphx tools (Zen, Silk)
- 🔥 **Same Features** - 100% feature parity with VitePress
...
- **Build Time**: Fast builds powered by Bun and Vite
- **Bundle Size**: Comparable to VitePress (~73KB gzipped)
- **Runtime**: Modern JavaScript runtime with Bun
- **HMR**: Quick hot module replacement

::: tip Benchmarks
We're working on comprehensive performance benchmarks. Early testing shows competitive performance with benefits from Bun's faster runtime.
:::
